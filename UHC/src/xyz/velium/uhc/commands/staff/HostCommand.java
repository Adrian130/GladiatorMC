...
public class HostCommand implements CommandExecutor {
    ...
    private void handleHost(Player player) {
        ...
        if(uhcPlayer.getPlayerStates() != PlayerStates.HOST) {
            ...
            uhcPlayer.setPlayerStates(PlayerStates.HOST);

            player.sendMessage(uhc.getPREFIX() + ChatColor.GREEN + "You are now in Host Mode");
            ...
            if(uhc.getGameStates() == GameStates.PREPARE || uhc.getGameStates() == GameStates.LOBBY) {
                ...
                player.sendMessage(uhc.getPREFIX() + ChatColor.RED + "You left the Host Mode");
                ...
                player.sendMessage(uhc.getPREFIX() + ChatColor.RED + "You can´t leave the Host Mode while the Scatter is running");
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String invoke, String[] arguments) {
        if(commandSender instanceof Player) {
            ...
            if(arguments.length == 0) {
                this.handleHost(player);
            } else {
                player.sendMessage(uhc.getPREFIX() + ChatColor.RED + "Usage: /host");
            }
        }
        return false;
    }
